...
class MovementPlugin(Plugin):
    def __init__(self, context):
        ...
        self._input_pub = self._node.create_publisher(String, 'input_topic', 10)
        
        # Create subscriber for position feedback
        self._uart_rx_sub = self._node.create_subscription(String, 'uart_receive', self._uart_feedback_callback, 10)
    ...
    def _connect_signals(self):
        ...
        self._widget.radioButton_small.toggled.connect(self._handle_increment_changed)
        self._widget.radioButton_medium.toggled.connect(self._handle_increment_changed)
        self._widget.radioButton_large.toggled.connect(self._handle_increment_changed)
